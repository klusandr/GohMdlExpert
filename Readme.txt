...
(beta)
   ...
   0.0.1.0 release;
   0.0.2.0 added Finnish;
   0.0.2.1 hotfix default LOD null, load soviet resource, load unpack resource;
   0.0.3.0 added united kingdom, various fixes;
   0.1.0.0  
    added mod support  
    added humanskin loading form game resources  
    added the ability to create make humanskin from elements of any faction  
    added models LOD overview  
    changed save principle  
    changed resource loading principle  
    removed fraction system, all resource show together now  
...
When the game resources are loaded will be available in the left column at the top, select the desired one.
...
If you select a texture file (mtl), the available materials for the selected texture are at the bottom of the right column. If you expand the texture file, the skin elements that use the current texture will be displayed. To add a new texture, click the "Add" button under the texture list overview window, and in the window that appears, in the "Textures" tab, select a texture suitable for all skin elements, and click "OK". To edit an existing texture, double-click on it. To select a texture that will be used when saving, simply select it in the list. Carefully check that all elements adequately display the selected texture.
...
If for some reason you do not have textures for any skin element, and there are no elements in the "Load materials" texture selection window, then you need to load the cache to display them. Select the item in the top menu "Edit->Caches", and "Load PLY model textures" if there are no textures for the elements, or "Load textures" if there are no textures in the selection window.

 You can report a bug here [https://forms.gle/1pJpmXkk4RSkcXfY9](https://forms.gle/1pJpmXkk4RSkcXfY9)
...
(beta)
   ...
   0.1.0.0  
    ...
    добавлена загрузка Humanskin из игровых ресурсов  
    добавлена возможность сохранения Humanskin из элементов разных фракций  
    добавлен обзор LOD-моделей  
    изменен принцип сохранения  
    изменен принцип загрузки ресурсов  
    удалена система фракций теперь все ply ресурсы отображаются вместе, без разделения  
...
Если выбрать файл текстуры (mtl), внизу правой колонки доступные материалы для выделенной текстуры. Если её развернуть, отобразиться элементы скина, использующие текущую текстуру. Для добавления новой текстуры, нажимаем кнопку "Add" под окном обзора списка текстур, и в появившемся окне, во вкладке "Textures", выбираем текстуру подходящую для всех элементов скина, и нажимаем "Ok". Для редактирования уже существующей текстуры, дважды на неё нажимаем. Что бы выбрать текстуру, которая будет использована при сохранении, просто выбираем её в списке. Внимательно проверяйте, что все элементы адекватно отображают выбранную текстуру.
...
Если у вас по какой-то причине ни для одного элемента скина нет текстур, а в окне выбора текстур "Load materials" нет элементов, то необходимо загрузить кэш для их отображения. Выбираем пункт в верхнем меню "Edit->Caches", и "Load PLY model textures" если нет текстур для элементов, или "Load textures" если нет текстур в окне их выбора.

 Сообщить об ошибке можно тут [https://forms.gle/1pJpmXkk4RSkcXfY9](https://forms.gle/1pJpmXkk4RSkcXfY9)
