...
namespace ConsoleApp4 {
    public class PlyReader {
        ...
        public static List<int> indicesList = new List<int>();

        public static void PlyRead (string fileName) {
            List<Point3D> points = new List<Point3D>();
            List<int> indicesList = new List<int>();

            var modelFileStream = new FileStream(fileName, FileMode.Open);
            byte[] readByte = new byte[64];

            modelFileStream.Read(readByte, 0, 8);
            var fileHeader = Encoding.ASCII.GetString(readByte, 0, 8);

            float[] readFloat = new float[6];
            modelFileStream.Read(readByte, 0, 24);
            for (int i = 0; i < readFloat.Length; i++) {
                readFloat[i] = BitConverter.ToSingle(readByte, i * 4);
            }

            

            while (modelFileStream.Position < modelFileStream.Length) {
                modelFileStream.Read(readByte, 0, 4);

                string chunkName = Encoding.ASCII.GetString(readByte, 0, 4);

                switch (chunkName) {
                    case "SKIN":
                        modelFileStream.Read(readByte, 0, 4);
                        int skinCount = BitConverter.ToInt32(readByte, 0);

                        for (int i = 0; i < skinCount; i++) {
                            var skinNameSize = modelFileStream.ReadByte();
                        }
                    ...
                    case "VERT":
                        modelFileStream.Read(readByte, 0, 8);
                        int vertexCount = BitConverter.ToInt32(readByte, 0);
                        int vertexDataSize = BitConverter.ToInt16(readByte, 4);
                        int vertexFlags = BitConverter.ToInt16(readByte, 6);
                        ...
                        for (int i = 0; i < vertexCount; i++) {
                            ...
                            point.x = BitConverter.ToSingle(vertexData, 1 * 4);
                            point.y = BitConverter.ToSingle(vertexData, 2 * 4);
                            point.z = BitConverter.ToSingle(vertexData, 3 * 4);
                    ...
                    case "INDX":
                        modelFileStream.Read(readByte, 0, 4);
                        int indicesCount = BitConverter.ToInt32(readByte, 0);
