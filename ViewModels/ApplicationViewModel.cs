...
namespace GohMdlExpert.ViewModels {
    public class ApplicationViewModel : ViewModelBase {
        ...
        public void OpenFile() {
            ...
            if (fileDialog.ShowDialog() ?? false) {
                ...
                foreach (var file in files) {
                    if (!file.Contains("head") && !file.Contains("rank_heer")) {
        ...
        private async Task ThroughModels(object? p) {
            var ranksFiles = Directory.GetFiles("F:\\SDK\\Content\\goh\\entity\\humanskin\\[germans]\\[ger_source]\\ger_tunic_pzwrap\\ger_ranks\\heer_ranks\\");
            ...
            await Task.Run(() => {
                foreach (var rankFile in ranksFiles) {
                    if (rankFile.Contains("leutnant")) {
                        break;
                    }

                    App.Current.Synchronize(() => {
                        models3DViewModel.Models.RemoveAt(models3DViewModel.Models.Count - 1);
                        models3DViewModel.Models.Add(models3DViewModel.OpenPlyFile(rankFile));
                    });
                    Thread.Sleep(500);
                }
            });
